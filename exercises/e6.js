...
export function getAsteroidsDiscoveredAfterYear(data, year) {
  const asteroidsDiscovered = data.asteroids.filter(asteroid => asteroid.discoveryYear > year);
  const asteroidNames = asteroidsDiscovered.map(asteroid => asteroid.name);
